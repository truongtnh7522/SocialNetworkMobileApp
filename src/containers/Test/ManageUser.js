...
const ManageUser = ({ navigation }) => {
  ...
  const hanldDltPost = async (id) => {
    ...
    try {
      // const data =;
      const res = await api.post(`https://truongnetwwork.bsite.net/api/admin/user/DeleteUserById?userId=${id}`);
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.scrollViewContent}>
      ...
      styles.element
        ...
        source={{ uri: dataPost[index]?.image}} 
        style={{ width: "30%", height: "60%" , borderRadius:10 }}
      />
      <View style={{
        width:"50%"
      }}>
      <Text>{dataPost[index]?.fullName}</Text>
      </View>
    <Feather name="trash-2" size={18} color="gray" onPress={() => hanldDltPost(dataPost[index]?.userId)}/>
      </View>
